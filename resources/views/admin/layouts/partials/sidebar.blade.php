    ...
    <ul>
        ...
        <li>
            <a href="{{ route('admin.support.index') }}" class="{{ request()->routeIs('admin.support.*') ? 'active' : '' }}">
                <i class="ph ph-chat-circle-dots"></i> Support Chats
                @if($unreadSupportCount > 0)
                    <span style="background: var(--red, #ef4444); color: white; padding: 2px 6px; border-radius: 12px; font-size: 11px; margin-left: auto; font-weight: bold;">{{ $unreadSupportCount }}</span>
                @endif
            </a>
        </li>
